...
remoter_t remoter;
float prx = 1.0f,pry = 1.0f,prz = 24.0f;
...
void RC_process(void)
{
    ...
    usart_printf("%f,%f,%f,%d,%d,%d,%d,%d\n",prx,pry,prz,remoter.rc.ch[0],remoter.rc.ch[1],remoter.rc.ch[2],remoter.rc.ch[3],output_mode);

    if(remoter.rc.ch[0] >= 1500 && output_mode == IF_MODE)
        prx += 0.06f;

    else if(remoter.rc.ch[0] <= 300 && output_mode == IF_MODE)
        prx -= 0.06f;

    if(remoter.rc.ch[2] >= 1500 && output_mode == IF_MODE)
        pry += 0.06f;

    else if(remoter.rc.ch[2] <= 300 && output_mode == IF_MODE)
        pry -= 0.06f;

    if(remoter.rc.ch[1] >= 1500 && output_mode == IF_MODE)
        prz += 0.06f;

    else if(remoter.rc.ch[1] <= 300 && output_mode == IF_MODE)
        prz -= 0.06f;
}