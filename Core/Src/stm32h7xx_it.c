...
void USART10_IRQHandler(void)
{
  /* USER CODE BEGIN USART10_IRQn 0 */
    if(__HAL_UART_GET_FLAG(&huart10,UART_FLAG_IDLE)==SET)
    {
        __HAL_UART_CLEAR_IDLEFLAG(&huart10);//清除空闲中断接受标志wei
        HAL_UART_DMAStop(&huart10);//关闭DMA接受
    }
    HAL_UART_Receive_DMA(&huart10,(uint8_t *)&Screen_Data,Screen_Length);//使能串口5 DMA接受
